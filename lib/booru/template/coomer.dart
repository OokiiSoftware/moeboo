...
abstract class ICoomer extends ABooru {
  ...
  ICoomer({super.options}) : super(BooruType.coomer);
  ...
  Future<List<Post?>> findPosts({required AlbumQuery query}) async {
    ...
    log.d(name, type.value, 'findPosts', 'uri', uri);
  ...
  Post? _getPost(Map map, Map item, int index) {
    ...
    String id = '${item['id']}';
    id = id.replaceAll(RegExp(r'[a-zA-Z!@#<>?":_`~;[\]\\|=+)(*&^%\s-]'), '').replaceAll(' ', '');
    if (id.length > 10) {
